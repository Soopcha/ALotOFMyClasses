...
public class CollectionsLearning {
    ...
    Integer - объектная обёртка на примитивным типом int

    Интерфейс - декралация возможностей, те описываем набор методов, которыми должны обладать потомки и те уже описывают их по своему


Иерархия
    -Iterable (интерфейс) (для того, чтобы можно было перебирать коллекции и мапы)
        -Collection (интерфейс)
            -List  (список)
            -Set   (множество, набор)
            -Queue (очередь)
        -Map (отдельно от Collection, тк разная структура) (карта, ассоциативный массив)
    ...
    public static void main(String[] args) {
        ...
        // Когда проваливаемся в LIST: extends - наследует (те потомки(классы наследники)(дети) реализуют всю функциональность родителя + добавляют своё)
        //<E> - это дженерик
